...
des<-svydesign(ids =~psu, strata = ~strata, weights = ~weight, data=nhanes_all, nest=T)
...
nhanes_healthy$zsleep<-scale(nhanes_healthy$sleephours)
nhanes_healthy$zwbc<-scale(nhanes_healthy$lbxwbcsi)
nhanes_healthy$zwbc1<-scale(nhanes_healthy$lbdlymno)
nhanes_healthy$zwbc2<-scale(nhanes_healthy$lbdbano)
nhanes_healthy$zwbc3<-scale(nhanes_healthy$lbdmono)
nhanes_healthy$zwbc4<-scale(nhanes_healthy$lbdneno)
nhanes_healthy$zwbc5<-scale(nhanes_healthy$lbdeono)
...
```{r}
library(tableone)
t0<-svyCreateTableOne(vars = c("d.event", "sleephours", "lbxwbcsi","lbdlymno","lbdbano","lbdmono", "lbdneno","lbdeono", "bmibrk", "male", "race_eth", "agebrk", "depress_cut", "poorhealth", "sl2"),  data= des)
print(t0, format = "p")

t1<-svyCreateTableOne(vars = c("sleephours", "lbxwbcsi","lbdlymno","lbdbano","lbdmono", "lbdneno","lbdeono", "bmibrk", "male", "race_eth", "agebrk", "depress_cut", "poorhealth", "sl2"), strata = "d.event", data= des, test = T)
print(t1, format = "p")
```
...
of1<-svyglm(d.event~zsleep, des, family=binomial )
mm1<-svyglm(zwbc~zsleep,des)
mm2<-svyglm(d.event~zwbc+zsleep,des, family=binomial )
...
of1<-svyglm(d.event~zsleep, des, family=binomial )
mm1<-svyglm(zwbc1~zsleep,des)
mm2<-svyglm(d.event~zwbc1+zsleep,des, family=binomial )
...
of1<-svyglm(d.event~zsleep, des, family=binomial )
mm1<-svyglm(zwbc2~zsleep,des)
mm2<-svyglm(d.event~zwbc2+zsleep,des, family=binomial )
...
of1<-svyglm(d.event~zsleep, des, family=binomial )
mm1<-svyglm(zwbc3~zsleep,des)
mm2<-svyglm(d.event~zwbc3+zsleep,des, family=binomial )
...
of1<-svyglm(d.event~zsleep, des, family=binomial )
mm1<-svyglm(zwbc4~zsleep,des)
mm2<-svyglm(d.event~zwbc4+zsleep,des, family=binomial )
...
med.fit<-svyglm(zwbc~zsleep, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit0<-svyglm(d.event~zsleep, family=binomial,des )

out.fit<-svyglm(d.event~zsleep+zwbc, family=binomial,des )
...
```{r,eval=FALSE,warning=FALSE, message=FALSE, results="hide"}
med.fit<-svyglm(zwbc1~zsleep, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit0<-svyglm(d.event~zsleep, family=binomial,des )

out.fit<-svyglm(d.event~zsleep+zwbc1, family=binomial,des )
...
```{r,eval=FALSE,warning=FALSE, message=FALSE, results="hide"}
med.fit<-svyglm(zwbc2~zsleep, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit0<-svyglm(d.event~zsleep, family=binomial,des )

out.fit<-svyglm(d.event~zsleep+zwbc2, family=binomial,des )
...
```{r,eval=FALSE,warning=FALSE, message=FALSE, results="hide"}
med.fit<-svyglm(zwbc3~zsleep, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit0<-svyglm(d.event~zsleep, family=binomial,des )

out.fit<-svyglm(d.event~zsleep+zwbc3, family=binomial,des )
...
```{r,eval=FALSE,warning=FALSE, message=FALSE, results="hide"}
med.fit<-svyglm(zwbc4~zsleep, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit0<-svyglm(d.event~zsleep, family=binomial,des )

out.fit<-svyglm(d.event~zsleep+zwbc4, family=binomial,des )
...
```{r,eval=FALSE,warning=FALSE, message=FALSE, results="hide"}
med.fit<-svyglm(zwbc5~zsleep, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit0<-svyglm(d.event~zsleep, family=binomial,des )

out.fit<-svyglm(d.event~zsleep+zwbc5, family=binomial,des )
...
med.fit2<-svyglm(zwbc~zsleep+male+bmibrk+race_eth+agebrk+poorhealth, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit2<-svyglm(d.event~zsleep+zwbc+male+bmibrk+race_eth+agebrk+poorhealth, family=binomial,des )
...
```{r,warning=FALSE,message=FALSE, results="hide"}
med.fit2<-svyglm(zwbc1~zsleep+male+bmibrk+race_eth+agebrk+poorhealth, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit2<-svyglm(d.event~zsleep+zwbc1+male+bmibrk+race_eth+agebrk+poorhealth, family=binomial,des )
...
```{r,warning=FALSE,message=FALSE, results="hide"}
med.fit2<-svyglm(zwbc4~zsleep+male+bmibrk+race_eth+agebrk+poorhealth, family=gaussian, des)

#of1<-glm(d.event~zsleep, data=nhanes_all, family=binomial, weights = weight/mean(weight, na.rm=T) )
out.fit2<-svyglm(d.event~zsleep+zwbc4+male+bmibrk+race_eth+agebrk+poorhealth, family=binomial,des )
